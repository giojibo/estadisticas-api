...
from estadisticas_api.models import PesoMensual
from estadisticas_api.serializers import pesoMensualSerializers


class PesoMensualListCreateAPIView(APIView):

    def get(self, request):
        # Obtener el parámetro 'paciente' desde la URL
        paciente = request.query_params.get('paciente', None)
        if paciente:
            registros = PesoMensual.objects.filter(paciente=paciente)
        else:
